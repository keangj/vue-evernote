...
const actions = {
  ...
  deleteTrashNote({ commit }, { noteId }) {
    return Trash.confirm({ noteId }).then(res => {
      commit("deleteTrashNote", { noteId });
      Message.success(res.msg);
    });
  },
  revertTrashNote({ commit, rootGetters }, { note }) {
    return Trash.revert({ noteId: note.id }).then(res => {
      // let notebook = rootGetters.notebooks.find(notebook => notebook.id == note.notebookId) || {}
      commit("deleteTrashNote", { noteId: note.id });
      Message.success(res.msg);
    });
  }
}
